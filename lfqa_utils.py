...
# Sparse index
###############
def make_es_index_snippets(es_client, passages_dset,
...
def query_es_index(question, es_client,
                   index_name="english_wiki_kilt_snippets_100w", n_results=10, min_length=20):
    q = question.lower()
    banned = [
        "how",
        "why",
...
class ELI5DatasetQARetriver(Dataset):
    def __init__(self, examples_array, extra_answer_threshold=3,
                 min_answer_length=64, training=True, n_samples=None):
...
class RetrievalQAEmbedder(torch.nn.Module):
    ...
    def embed_sentences_checkpointed(
        ...
        if checkpoint_batch_size < 0 or input_ids.shape[0] < checkpoint_batch_size:
            return self.sent_encoder(
                input_ids, attention_mask=attention_mask)[1]
        else:
    ...
    def forward(self, q_ids, q_mask, a_ids, a_mask, checkpoint_batch_size=-1):
        ...
        loss_qa = self.ce_loss(
            compare_scores, torch.arange(
                compare_scores.shape[1]).to(device))
        loss_aq = self.ce_loss(
            compare_scores.t(), torch.arange(
...
def make_qa_retriever_model(
    ...
    if from_file is not None:
        ...
        qa_embedder.load_state_dict(param_dict["model"], strict=False)
    ...
    for step, (batches,) in enumerate(zip(joint_iter)):
        ...
        if step % args.print_freq == 0:
            print(
                "{:2d} {:5d} of {:5d} \t L: {:.3f} \t -- {:.3f}".format(
                    e, step, len(
                        dataset_list[0]) // args.batch_size, loc_loss / loc_steps, time() - st_time,
                )
            )
            loc_loss = 0
            loc_steps = 0
...
def train_qa_retriever(qar_model, qar_tokenizer,
                       qar_train_dset, qar_valid_dset, qar_args):
    qar_optimizer = AdamW(
        qar_model.parameters(),
        lr=qar_args.learning_rate,
        eps=1e-8)
...
def make_qa_s2s_model(model_name="facebook/bart-large",
                      from_file=None, device="cuda:0"):
    tokenizer = AutoTokenizer.from_pretrained(model_name)
...
def make_qa_s2s_batch(qa_list, tokenizer, max_len=64,
                      max_a_len=360, device="cuda:0"):
    q_ls = [q for q, a in qa_list]
    a_ls = [a for q, a in qa_list]
    q_toks = tokenizer.batch_encode_plus(
        q_ls, max_length=max_len, pad_to_max_length=True)
...
def train_qa_s2s_epoch(model, dataset, tokenizer, optimizer,
                       scheduler, args, e=0, curriculum=False):
    model.train()
    # make iterator
    if curriculum:
...
def qa_s2s_generate(
    ...
    return [qa_s2s_tokenizer.decode(
        ans_ids, skip_special_tokens=True).strip() for ans_ids in generated_ids]


def qa_s2s_generate_two(
        question_doc,
        qa_s2s_model,
        qa_s2s_tokenizer,
        num_answers=1,
    ...
    model_inputs = make_qa_s2s_batch([(q, "A") for q in question_doc], qa_s2s_tokenizer, max_input_length,
                                     device=device, )
    n_beams = num_answers if num_beams is None else max(num_beams, num_answers)
    generated_ids = qa_s2s_model.generate(
        input_ids=model_inputs["input_ids"],
...
def make_keyword_dense_index(
    ...
    for i in range(n_batches):
        passages = [p for p in passages_dset[i *
                                             batch_size: (i + 1) * batch_size]["section_title"]]
        reps = embed_passages_for_retrieval(
            passages, tokenizer, qa_embedder, max_length, device)
        fp[i * batch_size: (i + 1) * batch_size] = reps
        if i % 50 == 0:
...
def evaluate_retriever(qa_list, retriever_func,
                       scoring_func, n_ret=10, verbose=False):
    ...
    return {"idf_recall": total_retriever_score /
            (i + 1), "retrieval_time": total_retriever_time / (i + 1)}
...
def query_mix_dense_index(
        question, qa_embedder, tokenizer, wiki_passages, wiki_index1, wiki_index2, weight=0.5, n_results=10,
        min_length=20, device="cuda:0"
):
    q_rep = embed_questions_for_retrieval(
        [question], tokenizer, qa_embedder, device=device)
    D1, I1 = wiki_index1.search(q_rep, 2 * n_results)
    D2, I2 = wiki_index2.search(q_rep, 2 * n_results)

    res_passages1 = [(int(i), weight * float(d)) for i, d in zip(I1[0], D1[0])]
    res_passages2 = [(int(i), (1 - weight) * float(d))
                     for i, d in zip(I2[0], D2[0])]

    # print(res_passages1,res_passages2)
    res_passages = {}
    for i in res_passages1:
        if i[0] not in res_passages:
            res_passages[i[0]] = i[1]
    for i in res_passages2:
        if i[0] not in res_passages:
            res_passages[i[0]] = i[1]
        else:
            res_passages[i[0]] += i[1]

    # print(res_passages,len(res_passages))
    res_passages = list(res_passages.items())
    res_passages.sort(key=lambda item: (item[1], item[0]), reverse=True)
    # print(res_passages)
    D = [i[1] for i in res_passages]
    res_passages = [wiki_passages[i[0]] for i in res_passages]
...
def query_qa_dense_index_nn(passage, qa_embedder, tokenizer,
                            wiki_passages, wiki_index, n_results=10, min_length=20):
